...
reportsController.get(
  ...
  async (request, response) => {
    ...
    try {
      console.log(`Received request for employeeId: ${employeeId}, startDate: ${startDate}, endDate: ${endDate}`);
      const reportData = await getTotalHoursWorkedByEmployeeByDateRange(employeeId, startDate, endDate);
...
reportsController.get('/all/range/:startDate/:endDate', async (request, response) => {
  const { startDate, endDate } = request.params;
  try {
    const timecards = await getTotalHoursWorkedByAllEmployeesByDateRange(startDate, endDate);
    ...
    console.error(`Error fetching timecards for all employees between ${startDate} and ${endDate}: ${err.message}`);
...
reportsController.get(
  ...
  if (!employeeId || isNaN(Number(employeeId))) {
    return response.status(400).json({ error: "Invalid or missing employeeId" });
  }

  try {
    // Call the getEmployeeSummaryById function for a single employee
    const report = await getEmployeeSummaryById(employeeId, period, startDate, endDate);

    // If report is empty, return 404
    if (!report || report.length === 0) {
      return response.status(404).json({ message: "No data found for the specified employee and date range" });
    }

    // Return the employee summary report
    return response.status(200).json(report);
  } catch (error) {
    return response.status(500).json({ error: 'Error generating employee summary report' });
  }
});
...
reportsController.get("/all/employee-summary", async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({ error: 'Error generating summary report for all employees' });
  }
});
